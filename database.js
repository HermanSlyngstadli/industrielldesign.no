var db = {"styret": [
	{
		"tittel":"Il Presidente",
		"img":"ilpresidente.png",
		"mail":"Ilpresidente.leonardo@gmail.com",
		"navn":"Thomas Kaland",
		"sitat":"Tears come from the heart and not the brain.",
		"beskrivelse":"This section should contain an introduction to the committee leaders as well as contact info. It should also have a description of the different committees. What they do, and how they contribute. Maybe we can write down some tales and legends in the history part as well."
	},
	{
		"tittel":"Il Vice Presidente",
		"img":"ilvicepresidente.png",
		"mail":"ilvicepresidente.leonardo@gmail.com",
		"navn":"Rannveig Carlsen",
		"sitat":"Simplicity is the ultimate sophistication.",
		"beskrivelse":"This section should contain an introduction to the committee leaders as well as contact info. It should also have a description of the different committees. What they do, and how they contribute. Maybe we can write down some tales and legends in the history part as well."
	},
	{
		"tittel":"L'Affari",
		"img":"l'affari.png",
		"mail":"bedriftskontakt.leonardo@gmail.com",
		"navn":"Magnus Lie Fridheim",
		"sitat":"Learning never exhausts the mind.",
		"beskrivelse":"This section should contain an introduction to the committee leaders as well as contact info. It should also have a description of the different committees. What they do, and how they contribute. Maybe we can write down some tales and legends in the history part as well."
	},
	{
		"tittel":"Responsabile Pubblicitario",
		"img":"responsabilepubblicitario.png",
		"mail":"markedsforing.leonardo@gmail.com",
		"navn":"Herman Slyngstadli",
		"sitat":"Art is never finished, only abondoned.",
		"beskrivelse":"This section should contain an introduction to the committee leaders as well as contact info. It should also have a description of the different committees. What they do, and how they contribute. Maybe we can write down some tales and legends in the history part as well."
	},
	{
		"tittel":"Direttore Artistico",
		"img":"diretoreartistico.png",
		"mail":"direttoreartistico.leonardo@gmail.com",
		"navn":"Tobias Wulvik",
		"sitat":"The human foot is a masterpiece of engineering and a work of art.",
		"beskrivelse":"This section should contain an introduction to the committee leaders as well as contact info. It should also have a description of the different committees. What they do, and how they contribute. Maybe we can write down some tales and legends in the history part as well."
	},
	{
		"tittel":"Il Vernissageur",
		"img":"ilvernissageur.png",
		"mail":"ilvernissageur.leonardo@gmail.com",
		"navn":"Ingrid Dahle",
		"sitat":"The noblest pleasure is the joy of understanding.",
		"beskrivelse":"This section should contain an introduction to the committee leaders as well as contact info. It should also have a description of the different committees. What they do, and how they contribute. Maybe we can write down some tales and legends in the history part as well."
	},
	{
		"tittel":"Il Evento Guida",
		"img":"ileventoguida.png",
		"mail":"ileventoguida.leonardo@gmail.com",
		"navn":"Gjøril Fjeldstad",
		"sitat":"Nothing strenghtens authority so much as silence.",
		"beskrivelse":"This section should contain an introduction to the committee leaders as well as contact info. It should also have a description of the different committees. What they do, and how they contribute. Maybe we can write down some tales and legends in the history part as well."
	},
	{
		"tittel":"Il Prima",
		"img":"ilprima.png",
		"mail":"ilprima.leonardo@gmail.com",
		"navn":"Victoria Fjellbirkeland",
		"sitat":"The greatest deception men suffer is from their own opinions.",
		"beskrivelse":"This section should contain an introduction to the committee leaders as well as contact info. It should also have a description of the different committees. What they do, and how they contribute. Maybe we can write down some tales and legends in the history part as well."
	}
],
"komite": [
	{
		"tittel":"bedkom",
		"ikon":"BedKom.png",
		"img":"",
		"beskrivelse":"Er du gira på å lære å arrangere workshops og bedriftpresentasjoner, skape nye kontakter og være Leonardos ansikt utad til relevante bedrifter? Da er Bedriftskomiteen noe for deg! Bedriftskomiteen, blant venner kalt BK, er komiteen som bringer inntekt til linjeforeningen samtidig som den skaper bånd mellom næringslivet og studentene på IPD. Med andre ord er BK en viktig komité hvor du får nyttig erfaring, og en mulighet til å danne kontakt med potensielle arbeidsgivere som er verdifull når du selv skal ut i jobb. I tillegg er det lite som matcher rushet du får når du lander en ny avtale og får penger i kassa til Leonardo! Hvis du er en engasjert student som vil selge Leonardo linjeforening for alt den er verdt, trenger vi akkurat deg til å søke BK!"
	},
	{
		"tittel":"arrkom",
		"ikon":"ArrKom.png",
		"img":"",
		"beskrivelse":"Vil du være med å arrangere Åretur, kurs, utflukter, julebord og mye mer for medlemmene til den flotte linjen vår? Bli med i arrangementskomiteen da vel! Nå har vi opptak, og det eneste kravet for at du skal få være med er at du er gira. Åreturen er noe man må få med seg, og den blir enda morsommere når du får være med å arrangere den selv. Ellers i året står vi relativt fritt til å arrangere akkurat det vi har lyst til. Her ligger forholdene til rette for å være kreative og få til masse morsomt! Det blir møter et par ganger i måneden eller etter behov. Dette er ikke et verv som tar mye tid, men det betyr ikke at det ikke er morsomt."
	},
	{
		"tittel":"redaksjonen",
		"ikon":"Redda.png",
		"img":"",
		"beskrivelse":"Redaksjonen består av en håndfull engasjerte medlemmer som jobber tett for å utgi Gløshaugens vakreste linjeforeningsmagasin to ganger i semesteret. TheSign har vunnet mang en pris for sitt fokus på substans, aktualitet, humor og white space. Vi forsøker å holde på en viss kontinuitet og helhet, samtidig som vi stadig evaluerer, itererer og fornyer innhold og uttrykk slik vi finner det for godt. Innholdet til hver utgave klekkes ut på den tradisjonelle kreativkvelden, hvor vi idémyldrer, fjaser, fyller disposisjonen og koser oss fælt. Videre har vi en arbeidskveld og ett lunsjmøte i uka. Etter hvert som deadline kryper nærmere gjenstår noen sene kvelder på IPD med innspurt og innslag av blant annet kaffe og pizza, flytting av pixler, runder rundt huset og krangling om fonter. Til slutt leser vi korrektur, sender til trykk og gleder oss til lanseringsstund for resten av Leonardo i idékafeen!"
	},
	{
		"tittel":"prkom",
		"ikon":"PRKom.png",
		"img":"",
		"beskrivelse":"På mange måter kan du se på PR-komitéen som Leonardos contour og highlights. Vi er ikke her for å sette vippe-extensions på Leonardo, men heller fremheve de allerede fantastiske kvalitetene til den flotte linjeforeningen vår. Vi jobber med alt fra gensere, plakater og grafisk profil til drift av nettsiden og sosiale medier. Vår oppgave er å hjelpe de andre komitéene med å kommunisere med studentene, og realisere visjonene deres. Samt også å få de andre linjeforeningene til å falme i skyggen av den vitruviske mann. I PR-komitéen vil du få lære og utfolde deg innen grafisk og strategisk markedsføring. Vi søker folk som liker å holde på med sosiale medier og/eller liker å bryne seg på en grafisk utfordring. Liker du heller å ta bilder eller filme, er du også velkommen hos oss!"
	},
	{
		"tittel":"festkom",
		"ikon":"FestKom.png",
		"img":"",
		"beskrivelse":"Festkomiteen steller i stand festene her på huset, alt fra halloween til gravøl. Som medlem av festkomiteen får du være med å bestemme hvordan fester vi skal arrangere her på huset. Forrige semester var hipsterfesten og sympatisfesten store slagere og nå er vi klare for flere. Det å være med i festkomiteen innebærer også å rigge i stand til festen på IPD. Her er det snakk om å stelle i stand baren, henge opp diskoball og lys, sette opp fotoboksen, og dekorere. Temafester er stort på IPD og for å bidra til riktig stemning er det viktig med dekorasjon og rekvisitter. Det sørger festkomiteen for!"
	},
	{
		"tittel":"fixkom",
		"ikon":"FixKom.png",
		"img":"",
		"beskrivelse":"Det nyeste komitétilskuddet i Leonardo-familien har arbeidsoppgaver som er både nærliggende og relevant for linja vi går på. Denne komiteen vil ganske enkelt holde på med ulike bygge-/fikseprosjekter, og vil få økonomisk støtte til sine skaperverk. Gruppa kommer til å utføre oppdrag for linjeforeningen, men også holde på med egne prosjekter. For å være med i denne enheten, stilles enkelte krav til kyndighet, men først og fremst er det interesse og skapervilje som er viktig. For øvrig vil gruppa inngå i en eventuell verkstedordning på kveldstid, og ha noe ansvar i forbindelse med denne. Alt som bygges vil kunne friske opp din portefølje; Dette vervet er med andre ord en ypperlig mulighet til å både bidra og skaffe seg aktuell erfaring."
	}
],
"tipsogtricks": [
	{
		"tittel":"",
		"img":"blyant.png"
	},
	{
		"tittel":"For å få utbetalt stipendet må du betale semesteravgift!",
		"img":"cash.png"
	},
	{
		"tittel":"Engasjer deg i et frivillig verv",
		"img":"hand.png"
	},
	{
		"tittel":"Huskutkledningstøy",
		"img":"batman.png"
	},
	{
		"tittel":"I Trondheim er det gjerne fire årstider på en dag. Husk allværsjakka.",
		"img":"paraply.png"
	},
	{
		"tittel":"Delta på Leonardos arrangmenter",
		"img":"leo.png"
	},
	{
		"tittel":"Last ned Mazemapappen for å navigere deg rundt på NTNU.",
		"img":"maze.png"
	},
	{
		"tittel":"Bli med i NTNUI.",
		"img":"ntnui.png"
	},
	{
		"tittel":"Vær flink til å planlegge før du handler.",
		"img":"liste.png"
	},
	{
		"tittel":"Kjøp brukte pensumbøker av studenter i kullene over deg.",
		"img":"bok.png"
	},
	{
		"tittel":"",
		"img":"saks.png"
	}
],
"arrangementer": [
	{
		"tittel":"Halloween",
		"img":"",
		"tekst":"This section should contain an introduction to the committee leaders as well as contact info. It should also have a description of the different committees. What they do, and how they contribute. Maybe we can write down some tales and legends in the history part as well.",
		"date":"03.02",
		"time":"20:00",
		"location":"IPD"
	},
	{
		"tittel":"Gravøl",
		"img":"",
		"tekst":"Det er duket for årets gravøl. Vi sier farvel til de som har forlatt oss.",
		"date":"12.3",
		"time":"21:00",
		"location":"IPD"
	},
	{
		"tittel":"Festfest",
		"img":"",
		"tekst":"Tro det eller ei, men dette er en fest. Det blir bra.",
		"date":"20.3",
		"time":"17:00",
		"location":"Skiboli"
	},
	{
		"tittel":"Forelesning med Knut Hamsun",
		"img":"",
		"tekst":"Han snakker om hvor kjipt det er å være kjip, og hans erfaringer med svakhet.",
		"date":"04.04",
		"time":"12:00",
		"location":"Hovedbygningen"
	},
	{
		"tittel":"17. mai frokost",
		"img":"",
		"tekst":"Vi har selvfølgelig frokost på nasjonaldagen også i år. Dette blir gøyalt.",
		"date":"17.05",
		"time":"19:00",
		"location":"IPD"
	}
],
"karriere": [
	{
		"tittel":"Making Waves søker nye interaksjonsdesignere",
		"img":"makingwaves.png",
		"tekst":"Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat"
	},
	{
		"tittel":"EGGS vil ha deg med på laget!",
		"img":"eggs.png",
		"tekst":"Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat"
	},
	{
		"tittel":"Accenture vokser stadig. Ble med du og da vel.",
		"img":"accenture.png",
		"tekst":"Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat"
	},
	{
		"tittel":"Søk sommerjobb hos Blank Oslo!",
		"img":"blank.png",
		"tekst":"Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat"
	}
],
"hvavikan": [
	"Produktdesign",
	"Produktutvikling",
	"Brukergrensesnitt",
	"Interaksjonsdesign",
	"Prosjektledelse",
	"Prosjektstrategier",
	"Brukeranalyse",
	"3D-modellering",
	"Visualisering",
	"Grafisk Design"
],
"prosjekter": [
	{
		"tittel":"KALIS3000",
		"img":"leo.png",
		"intro":"",
		"tekst":"",
		"klasse":"3. klasse",
		"year":"2019",
		"eier":"",
	},
	
	{
		"tittel":"zZz",
		"img":"bed.png",
		"intro":"",
		"tekst":"",
		"klasse":"58",
		"year":"3009",
		"eier":"",
	},
	
	{
		"tittel":"",
		"img":"studenter.png",
		"intro":"",
		"tekst":"",
		"klasse":"",
		"year":"",
		"eier":"",
	},
	
	{
		"tittel":"",
		"img":"forming.png",
		"intro":"",
		"tekst":"",
		"klasse":"",
		"year":"",
		"eier":"",
	},
	
	{
		"tittel":"",
		"img":"leo.png",
		"intro":"",
		"tekst":"",
		"klasse":"",
		"year":"",
		"eier":"",
	},
	
	{
		"tittel":"",
		"img":"leologo.png",
		"intro":"",
		"tekst":"",
		"klasse":"",
		"year":"",
		"eier":"",
	},
	
	{
		"tittel":"",
		"img":"",
		"intro":"",
		"tekst":"",
		"klasse":"",
		"year":"",
		"eier":"",
	},
	
	{
		"tittel":"",
		"img":"",
		"intro":"",
		"tekst":"",
		"klasse":"",
		"year":"",
		"eier":"",
	},
	
	{
		"tittel":"",
		"img":"",
		"intro":"",
		"tekst":"",
		"klasse":"",
		"year":"",
		"eier":"",
	},
	
	{
		"tittel":"",
		"img":"",
		"intro":"",
		"tekst":"",
		"klasse":"",
		"year":"",
		"eier":"",
	},
	
	{
		"tittel":"",
		"img":"",
		"intro":"",
		"tekst":"",
		"klasse":"",
		"year":"",
		"eier":"",
	},
	
]
};